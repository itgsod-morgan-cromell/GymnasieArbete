    ...
    def handle_event(self, event):
        ...
        if etype == pygame.MOUSEMOTION:
            if 0 < self.rect.x > self.offset.w or 0 < self.rect.y > self.offset.h:
                ...
                new_grid_x = int((event.pos[0]-3) / 32)
                new_grid_y = int((event.pos[1]-26) / 32)
                if (old_grid_x, old_grid_y) != (new_grid_x, new_grid_y):
                    post_event(GUI_TOOLTIP_CLEAR)
                    self.interact(new_grid_x, new_grid_y)
            self.rect.x = copy.copy(event.pos[0]-3)
            self.rect.y = copy.copy(event.pos[1]-26)
...
class Tooltip(Gui):
    ...
    def update(self, world, offset):
        self.world = world
        self.x = pygame.mouse.get_pos()[0]-3
        self.y = pygame.mouse.get_pos()[1] - self.image.height - 26
