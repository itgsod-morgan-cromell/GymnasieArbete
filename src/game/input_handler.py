...
class Input():
    ...
    def GetControl(self, name):
        ...
        for map in mappings:
            ...
            if source == 'buttonstate':
                stick, button = args
                result += self.GetButtonState(stick, button)
            ...
            if (source == 'stickstate'):
                stick, axis, direction, threshold = args
                result += self.GetStickState(stick, axis, direction, threshold)
            if (source == 'mousedir'):
                axis, = args
    ...
    def GetStickState(self, stick, axis, direction, threshold):
        if self.stickCount <= stick: return False
        value = self.stickStates[stick]['axes'][axis]
        return value * direction > threshold
    ...
    def GetButtonState(self, stick, button):
        if self.stickCount <= stick: return False
        return self.stickStates[stick]['stick'].get_button(button)
